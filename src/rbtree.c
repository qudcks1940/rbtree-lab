...
void rbtree_insert_fixup(rbtree* t, node_t* current) {
    // 첫 번째 코드가 실패하는 이유는 회전이 발생한 후에도 
    // 여전히 이전의 parent, grand 변수를 사용하기 때문. 
    // 회전 후에는 부모와 조부모 노드의 구조가 변경되므로, 
    // 변수에 저장된 값은 더 이상 유효하지 않다. 
    // 반면에 두 번째 코드는 매번 current->parent, 
    // current->parent->parent를 직접 참조하기 때문에, 
    // 트리 구조가 회전으로 변경된 후에도 
    // 항상 올바른 값을 참조할 수 있어서 성공적으로 동작
...
node_t* rbtree_find(const rbtree* t, const key_t key) {
    ...
    if (start == t->nil) {
        // t->root가 t->nil인 경우는
        // init함수는 호출해놓고 삽입이 된 적이 없는 경우니까
        // 루트에는 아무 값도 없다는 뜻이므로
        // NULL을 return 해야함.
        return NULL;
    }
    ...
    // 찾고자 하는 값이 없으면 NULL을 return할 수 있도록.
    return NULL;
}
